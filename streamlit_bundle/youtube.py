...
# Tentar importar bibliotecas opcionais para Streamlit Cloud
try:
...
def try_alternative_extraction(input_path, output_path, quality='standard'):
    """Tenta extrair áudio usando métodos alternativos quando FFmpeg não está disponível."""
    try:
        st.info("🔄 Tentando métodos alternativos de extração de áudio...")

        # Tentar usar moviepy primeiro
        if MOVIEPY_AVAILABLE:
            try:
                st.info("🎬 Usando MoviePy para extração de áudio...")
                ...
                if os.path.exists(output_path):
                    st.success("✅ Áudio extraído com sucesso usando MoviePy!")
                    return True
                else:
                    st.warning("⚠️ MoviePy não conseguiu criar o arquivo de áudio")
            except Exception as moviepy_error:
                st.warning(f"MoviePy falhou: {moviepy_error}")

        # Tentar usar ffmpeg-python como segunda opção
        if FFMPEG_PYTHON_AVAILABLE:
            try:
                st.info("🎵 Tentando ffmpeg-python...")
                ...
                if os.path.exists(output_path):
                    st.success("✅ Áudio extraído com sucesso usando ffmpeg-python!")
                    return True
                else:
                    st.warning("⚠️ ffmpeg-python não conseguiu criar o arquivo de áudio")
            except Exception as ffmpeg_error:
                st.warning(f"ffmpeg-python falhou: {ffmpeg_error}")

        # Se chegou aqui, nenhuma alternativa funcionou
        st.error("❌ Não foi possível extrair áudio com nenhum método disponível")
        st.info("💡 **Soluções recomendadas:**")
        st.info("1. **Use arquivos de áudio** (.wav, .mp3) diretamente em vez de vídeos")
        st.info("2. **Converta localmente:** Use ferramentas como VLC, FFmpeg ou online converters")
        st.info("3. **Exemplo de conversão:** ffmpeg -i video.mp4 audio.wav")
        return False

    except Exception as e:
        st.error(f"❌ Erro geral na extração alternativa: {e}")
        return False
...
def extract_audio_with_ffmpeg(input_path, output_path, quality='standard'):
    ...
    try:
        ...
        streamlit_headless = os.environ.get('STREAMLIT_SERVER_HEADLESS', 'false').lower()
        streamlit_base_url = os.environ.get('STREAMLIT_SERVER_BASE_URL', '')
        is_headless = streamlit_headless == 'true'
        is_streamlit_app = 'streamlit.app' in streamlit_base_url
        is_streamlit_cloud = is_headless or is_streamlit_app

        # Verificações adicionais para Streamlit Cloud
        is_cloud_via_hostname = False
        try:
            import socket
            hostname = socket.gethostname()
            is_cloud_via_hostname = 'streamlit' in hostname.lower()
        except:
            pass

        # Verificação adicional: tentar executar um comando que requer privilégios
        is_cloud_via_privileges = False
        try:
            # Tenta executar um comando que normalmente requer sudo/root
            result = subprocess.run(['id', '-u'], capture_output=True, text=True, timeout=5)
            # Se o UID não for 0 (root), pode ser um ambiente restrito
            is_cloud_via_privileges = result.returncode == 0 and result.stdout.strip() != '0'
        except:
            is_cloud_via_privileges = True  # Se falhar, assume que é ambiente restrito

        is_streamlit_cloud = is_streamlit_cloud or is_cloud_via_hostname or is_cloud_via_privileges

        if is_streamlit_cloud:
            # No Streamlit Cloud, tentar usar moviepy ou ffmpeg-python
            st.info("☁️ Detectado Streamlit Cloud - usando método alternativo...")
            ...
            try:
                ...
                if MOVIEPY_AVAILABLE:
                    ...
                    st.warning("⚠️ MoviePy não disponível, tentando ffmpeg-python...")
            ...
            try:
                ...
                if FFMPEG_PYTHON_AVAILABLE:
                    ...
                    st.error("❌ Nem MoviePy nem ffmpeg-python estão disponíveis no Streamlit Cloud")

            except Exception as ffmpeg_error:
                st.error(f"❌ Erro no ffmpeg-python: {ffmpeg_error}")

            # Se chegou aqui, nenhuma alternativa funcionou
            st.error("❌ Não foi possível extrair áudio no Streamlit Cloud")
            st.info("💡 **Solução:** Use arquivos de áudio (.wav, .mp3) diretamente em vez de vídeos")
            st.info("📋 **Para vídeos:** Converta para áudio primeiro usando ferramentas locais")
            return False

        # Método tradicional para ambientes locais
        ...
        if not shutil.which('ffmpeg'):
            ...
            try:
                ...
                if system == 'windows':
                    # No Windows, tenta múltiplas opções de instalação
                    try:
                        # Tenta winget primeiro (Windows 10/11)
                        st.info("🔄 Tentando instalar via winget...")
                        ...
                        if result.returncode == 0:
                            ...
                            st.info("🔄 Winget falhou, tentando chocolatey...")
                            result = subprocess.run(['choco', 'install', 'ffmpeg', '-y'],
                                                  capture_output=True, text=True, timeout=300)
                            if result.returncode == 0:
                                st.success("✅ FFmpeg instalado com sucesso via chocolatey!")
                            else:
                                # Tenta scoop como terceira opção
                                st.info("🔄 Chocolatey falhou, tentando scoop...")
                                result = subprocess.run(['scoop', 'install', 'ffmpeg'],
                                                      capture_output=True, text=True, timeout=300)
                                if result.returncode == 0:
                                    st.success("✅ FFmpeg instalado com sucesso via scoop!")
                                else:
                                    # Última tentativa: download direto
                                    st.info("🔄 Tentando download direto...")
                                    try:
                                        ...
                                        if ffmpeg_dir:
                                            ...
                                            st.success("✅ FFmpeg instalado e configurado temporariamente!")
                                        else:
                                            raise Exception("FFmpeg não encontrado no arquivo baixado")

                                    except Exception as download_error:
                                        st.error(f"❌ Falha no download direto: {download_error}")
                                        # Quando a instalação falha, tentar alternativas antes de desistir
                                        st.warning("🔄 Instalação falhou, tentando métodos alternativos...")
                                        return try_alternative_extraction(input_path, output_path, quality)

                    except subprocess.TimeoutExpired:
                        st.error("❌ Timeout na instalação automática.")
                        # Quando timeout, tentar alternativas
                        st.warning("🔄 Timeout na instalação, tentando métodos alternativos...")
                        return try_alternative_extraction(input_path, output_path, quality)
                    except Exception as win_error:
                        st.error(f"❌ Erro na instalação Windows: {win_error}")
                        # Quando erro na instalação, tentar alternativas
                        st.warning("🔄 Instalação falhou, tentando métodos alternativos...")
                        return try_alternative_extraction(input_path, output_path, quality)

                elif system == 'linux':
                    # No Linux, usa apt (mas pode falhar no Streamlit Cloud)
                    st.info("🔄 Tentando instalar via apt...")
                    ...
                    if result.returncode == 0:
                        ...
                        # Quando falha no Linux, tentar alternativas (provavelmente Streamlit Cloud)
                        st.warning("🔄 Instalação falhou, tentando métodos alternativos...")
                        return try_alternative_extraction(input_path, output_path, quality)
                    ...
                    if result.returncode == 0:
                        ...
                        # Quando falha no macOS, tentar alternativas
                        st.warning("🔄 Instalação falhou, tentando métodos alternativos...")
                        return try_alternative_extraction(input_path, output_path, quality)

                else:
                    st.error(f"❌ Sistema operacional não suportado para instalação automática: {system}")
                    # Sistema não suportado, tentar alternativas
                    st.warning("🔄 Sistema não suportado, tentando métodos alternativos...")
                    return try_alternative_extraction(input_path, output_path, quality)

            except Exception as install_error:
                st.error(f"❌ Erro durante instalação automática: {install_error}")
                # Qualquer erro na instalação, tentar alternativas
                st.warning("🔄 Instalação falhou completamente, tentando métodos alternativos...")
                return try_alternative_extraction(input_path, output_path, quality)

        # Verifica novamente se o FFmpeg está disponível após tentativa de instalação
        if not shutil.which('ffmpeg'):
            st.error("❌ FFmpeg ainda não está disponível após tentativa de instalação.")
            st.info("📋 **Instruções manuais de instalação:**")
            st.code("Windows: winget install --id Gyan.FFmpeg")
            st.code("Ou baixe de: https://ffmpeg.org/download.html")
            st.code("Adicione o ffmpeg.exe ao PATH do sistema")
            return False
...
def split_audio(audio_path, chunk_length_ms=120000, min_chunk_length_ms=30000):  # 2 minutos por chunk, mínimo 30s
    """Divide áudio em chunks menores, garantindo duração mínima para o último chunk."""
    try:
        audio = AudioSegment.from_file(audio_path)
        total_duration = len(audio)

        # Se o áudio for menor que o chunk mínimo, retorna como um único chunk
        if total_duration <= min_chunk_length_ms:
            return [audio]
        ...
        while current_position < total_duration:
            # Para o último chunk, garantir duração mínima
            remaining = total_duration - current_position
            if remaining <= min_chunk_length_ms and len(chunks) > 0:
                # Se o último chunk seria muito curto, adiciona ao chunk anterior
                if len(chunks) > 0:
                    # Remove o último chunk e recria com duração maior
                    last_chunk = chunks.pop()
                    last_start = current_position - len(last_chunk)
                    # Cria um chunk que vai até o final do áudio
                    chunk = audio[last_start:total_duration]
                    chunks.append(chunk)
                break
            else:
                # Chunk normal
                end_position = min(current_position + chunk_length_ms, total_duration)
...
def transcribe_audio_chunk(audio_chunk, recognizer, chunk_index, language='auto'):
    ...
    try:
        ...
        # Debug: verificar se o arquivo foi criado e tem tamanho adequado
        if os.path.exists(temp_file):
            file_size = os.path.getsize(temp_file)
            st.info(f"📁 Arquivo temporário criado: {temp_file} ({file_size} bytes)")
        else:
            st.error(f"❌ Arquivo temporário não foi criado: {temp_file}")
            return f"[Erro] Falha ao criar arquivo temporário"

        # Verificar se o chunk tem duração suficiente
        chunk_duration = len(audio_chunk) / 1000.0  # duração em segundos
        if chunk_duration < 3.0:  # chunks muito curtos podem ter problemas
            st.warning(f"⚠️ Segmento {chunk_index + 1} muito curto ({chunk_duration:.1f}s) - pode afetar a qualidade")

        # Debug: log da duração do chunk
        st.info(f"🎵 Processando segmento {chunk_index + 1} - Duração: {chunk_duration:.1f}s")

        with sr.AudioFile(temp_file) as source:
            # Ajustar para ruído ambiente (menos tempo para chunks curtos)
            adjust_duration = min(0.5, chunk_duration / 2)
        ...
        for lang in languages_to_try:
            try:
                # Debug: tentar transcrição
                st.info(f"🎯 Tentando idioma {lang} para segmento {chunk_index + 1}")

                # Primeira tentativa: Google Speech Recognition
                text = recognizer.recognize_google(audio_data, language=lang)
                if text.strip():  # Se conseguiu algum texto
                    result = f"[{lang}] {text}"
                    st.info(f"✅ Sucesso com {lang}: {len(text)} caracteres")
                    return result
                else:
                    transcription_attempts.append(f"{lang}: texto vazio")
            except sr.UnknownValueError:
                transcription_attempts.append(f"{lang}: UnknownValueError")
                continue
            except sr.RequestError as e:
                transcription_attempts.append(f"{lang}: RequestError - {e}")
                continue

        # Se todas as tentativas falharam, mostrar debug
        st.warning(f"❌ Todas as tentativas de transcrição falharam para segmento {chunk_index + 1}: {transcription_attempts}")
    ...
    try:
        ...
        if source_type == "youtube":
            ...
            if extract_audio_with_ffmpeg(temp_input, audio_file, audio_quality):
                ...
                # Verificar duração do áudio extraído
                try:
                    from pydub import AudioSegment
                    extracted_audio = AudioSegment.from_file(audio_file)
                    audio_duration = len(extracted_audio) / 1000.0  # em segundos
                    st.info(f"✅ Áudio extraído: {audio_duration:.1f}s de duração")
                except Exception as duration_error:
                    st.warning(f"⚠️ Não foi possível verificar duração do áudio: {duration_error}")
        ...
        # Debug: mostrar informações dos chunks
        for i, chunk in enumerate(chunks):
            chunk_duration = len(chunk) / 1000.0
            st.info(f"📊 Segmento {i+1}: {chunk_duration:.1f} segundos")
