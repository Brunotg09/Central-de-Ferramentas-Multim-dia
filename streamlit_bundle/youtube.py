                ...
                try:
                    ...
                    try:
                        ...
                        if removal_mode == 'Conservador (preserva texto)':
                            ...
                            kernel = _np.ones((1,1), dtype=_np.uint8)  # Minimal kernel
                            # Skip opening to preserve small details like text
                            # Only do minimal closing to fill tiny holes
                            alpha_clean = ndimage.binary_closing(alpha > 240, structure=kernel).astype(_np.uint8) * 255
                        elif removal_mode == 'Agressivo (melhor remoção)':
                            # More aggressive morphological operations
                            kernel = _np.ones((3,3), dtype=_np.uint8)
                            alpha_clean = ndimage.binary_opening(alpha > 128, structure=kernel).astype(_np.uint8) * 255
                            alpha_clean = ndimage.binary_closing(alpha_clean > 128, structure=kernel).astype(_np.uint8) * 255
                        else:  # Padrão
                            # Use simpler morphological operations to avoid Cholesky warnings
                        ...
                        # scipy not available, skip morphological operations
                        st.info("ℹ️ SciPy não disponível - pulando refinamento morfológico")
        ...
        if not CANVAS_AVAILABLE:
            ...
            if canvas_result is not None and hasattr(canvas_result, 'image_data') and getattr(canvas_result, 'image_data', None) is not None:
                ...
                try:
                    ...
                    # Detecta pixels desenhados: baixa opacidade OU pixels não-brancos
                    # Isso captura tanto desenhos com alpha quanto desenhos coloridos
                    drawn_pixels = (alpha > 20) | ((r < 250) | (g < 250) | (b < 250))
                    ...
                    if mask.any():
                        ...
                        if mask_bool is not None and mask_bool.any():
                            ...
                            with col1:
                                ...
                                st.image(preview_img, caption="Prévia (áreas vermelhas serão editadas)", use_container_width=True)
